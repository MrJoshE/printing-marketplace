...
def worker(mock_provider, mock_repo, in_memory_bus):
    ...
    worker._run_pipeline = MagicMock()
    return worker


@pytest.mark.asyncio
async def test_worker_happy_path(worker, in_memory_bus, mock_repo, mock_provider):
    """
    ...
    worker._run_pipeline.return_value = success_result
    ...
    mock_provider.store_file.assert_called_once()

    # Verify DB Updated
    mock_repo.complete_file_validation.assert_called_with("file_abc", "list_xyz", "user_1/list_xyz/file_abc.webp")
...
async def test_worker_transient_failure_retries(worker, mock_provider):
    ...
    mock_provider.store_file.side_effect = Exception("S3 Connection Reset")
...
async def test_worker_validation_failure(worker, mock_repo):
    ...
    worker._run_pipeline.return_value = fail_result
    ...
    mock_repo.mark_file_failed.assert_called_with("file_abc", "Image too large")
